...
// host with `LMOD_SKIP_BUILD=1 bun run modules:pack`. Keeping the plan derived
...
for (const dir of packableModules()) {
  ...
  lines.push(`cargo build --profile release-lmod -p ${pkg} --bin ${bin}${feat}${targetArg}`);
}
