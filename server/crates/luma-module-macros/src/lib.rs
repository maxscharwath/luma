...
//! ```
//!
//! into `pub const MODULE: EmbeddedModule = luma_module_sdk::embedded_module!();`.
...
pub fn embedded_module(_input: TokenStream) -> TokenStream {
    ...
    // caller has in scope (`use luma_module_sdk::EmbeddedModule` for the modules
    // above the facade, `use luma_module_manifest::EmbeddedModule` for the ones
