...
pub struct LogEntry {
    ...
    pub level: String,
    /// Rust tracing target for core lines (`luma_engine::infra::watch`),
    /// empty for module lines (their target stays in the message).
    pub target: String,
    /// `core` or the module id (`dev.luma.vpn`).
    pub source: String,
    pub message: String,
}
...
mod tests {
    ...
    fn module_line_parses_level_and_strips_timestamp() {
        ...
        buf.push_module_line("dev.luma.vpn", "2026-07-16T07:39:03.457400Z  WARN luma_vpn: wireguard bridge exited; restarting in 5s");
        let got = buf.snapshot(10, None, None, None);
        assert_eq!(got.len(), 1);
        assert_eq!(got[0].level, "warn");
        assert_eq!(got[0].source, "dev.luma.vpn");
        assert_eq!(got[0].message, "luma_vpn: wireguard bridge exited; restarting in 5s");
    }
    ...
    fn module_line_scrubs_ansi() {
        ...
        buf.push_module_line(
            "dev.luma.remote",
            "\x1b[2m2026-07-16T07:58:29.127560Z\x1b[0m \x1b[32m INFO\x1b[0m \x1b[2mluma_module_runtime\x1b[0m\x1b[2m:\x1b[0m module process starting \x1b[3mmodule\x1b[0m\x1b[2m=\x1b[0mdev.luma.remote",
        );
        let got = buf.snapshot(10, None, None, None);
        assert_eq!(got[0].level, "info");
        assert_eq!(
            got[0].message,
            "luma_module_runtime: module process starting module=dev.luma.remote"
        );
    }
