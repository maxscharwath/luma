...
The backend lives entirely in this module's own `server/` crate (`luma-remote`): the `ServerModule`, its admin routes, and the managed Cloudflare Tunnel (`cloudflared`) connector. It reaches the app only through the `HostCtx` seam.
