//! The Downloads module as a standalone process (its `.lmod` entrypoint).
//!
...
use luma_torrent::{DownloadDb, DownloadManager};
...
async fn main() -> anyhow::Result<()> {
    ...
    let extra = luma_port_bridge::download_routes::<RemoteHost>(grab, ledger)
        .merge(luma_port_bridge::downloadvpn_routes::<RemoteHost>(dc_vpn));
    ...
    luma_module_runtime::serve(
        ...
        vec![
            luma_torrent::server_module::<RemoteHost>(),
            luma_transmission::server_module::<RemoteHost>(),
            luma_qbittorrent::server_module::<RemoteHost>(),
        ],
        extra,
    )
    .await
