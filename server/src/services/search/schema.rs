//! The tantivy schema + the shared LUMA text analyzer.
//!
//! Every text field is tokenized by the `luma` analyzer (lowercase + diacritic
...
/// Name the LUMA analyzer is registered under on every index.
pub(super) const ANALYZER: &str = "luma";
...
/// A `luma`-analyzed, indexed (but not stored) text field.
...
/// Create a fresh in-RAM index from `schema` with the `luma` analyzer registered.
