...
use crate::model::{ContinueItem, Kind, ProgressEntry};
...
mod tests {
    ...
    fn season_cast_round_trips() {
        ...
        assert!(crate::db::seasons_with_cast(&pool, "s1").unwrap().is_empty());
        let cast = vec![CastMember {
            name: "Alice".into(),
            character: Some("Lead".into()),
            profile_url: None,
        }];
        crate::db::set_season_cast(&pool, "s1", 1, &cast).unwrap();
        crate::db::set_season_cast(&pool, "s1", 1, &cast).unwrap(); // idempotent upsert
        assert!(crate::db::seasons_with_cast(&pool, "s1").unwrap().contains(&1));
        let casts = crate::db::season_casts(&pool, "s1").unwrap();
